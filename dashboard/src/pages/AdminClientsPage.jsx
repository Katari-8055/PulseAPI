        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4 hover:border-slate-700 transition-colors group">
            <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-lg bg-indigo-600/10 flex items-center justify-center ring-1 ring-indigo-500/20">
                ...
                <span className={cn(
                    ...
                    <span className={cn('w-1.5 h-1.5 rounded-full', client.isActive ? 'bg-green-400' : 'bg-slate-500')} />
                    {client.isActive ? 'Active' : 'Inactive'}
                </span>
            </div>
            <p className="text-xs text-slate-500 mb-4 line-clamp-2">{client.description || 'No description provided.'}</p>
            <div className="flex items-center justify-between text-xs text-slate-600">
                <span>{new Date(client.createdAt).toLocaleDateString()}</span>
                <Link
                    to={`/admin/clients/${client._id}`}
                    className="flex items-center gap-1 text-indigo-400 hover:text-indigo-300 transition-colors opacity-0 group-hover:opacity-100"
                >
    ...
    return (
        <div className="p-6 max-w-6xl mx-auto space-y-6">
            <div className="flex items-start justify-between">
                <div>
                    <h1 className="text-xl font-semibold text-slate-100">All Clients</h1>
                    <p className="text-sm text-slate-500 mt-1">Manage all registered client organizations</p>
                </div>
            </div>
            ...
            {isPending ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center">
                    ...
                    <h3 className="text-sm font-medium text-slate-300 mb-1">No clients yet</h3>
                    <p className="text-xs text-slate-600">Clients will appear here once they register.</p>
                </div>
            ) : (
